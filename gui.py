...
class MilitaryApp:
    ...
    def create_widgets(self):
        ...
        # Кнопка для перекату
        Button(self.root, text="Перекат",width=18, font=("Stencil", 12, "bold"), command=self.show_relocation_window, bg="#556B2F", fg="white").place(x=1000, y=745)
    ...
    def show_relocation_window(self):
        # Створюємо нове вікно для перекату
        relocate_window = Toplevel(self.root)
        relocate_window.title("Перекат Позиції")
        relocate_window.geometry("300x400")
        relocate_window.config(bg="#2F4F2F")

        # Заголовок
        relocate_label = Label(relocate_window, text="Оберіть позицію для перекату", font=("Stencil", 14, "bold"), bg="#2F4F2F", fg="white")
        relocate_label.pack(pady=10)

        # Комбобокс для вибору позиції
        position_combobox = ttk.Combobox(relocate_window, values=self.units_position, font=("Arial", 10))
        position_combobox.pack(pady=10)
        
        # Кнопка для підтвердження вибору
        relocate_button = Button(relocate_window, text="Перекат", font=("Stencil", 12, "bold"), bg="#556B2F", fg="white", command=lambda: self.relocate_position(position_combobox.get(), relocate_window))
        relocate_button.pack(pady=10)

    def relocate_position(self, position_name, window):
        # Закриваємо вікно після підтвердження
        window.destroy()

        # Шукаємо координати для вибраної позиції
        position_row = self.db.fetch_all("SELECT lat_ch, long_ch FROM position WHERE name = ?", (position_name,))
        
        if position_row:
            lat_ch, lon_ch = position_row[0]
            lat_ch, lon_ch = float(lat_ch), float(lon_ch)

            # Видаляємо стару мітку для цієї позиції (якщо вона є)
            old_marker = self.target_markers.get(position_name)
            if old_marker:
                self.map_widget.delete(old_marker[1])  # Видалення мітки за її ID

            # Додаємо нову мітку на карту
            new_marker = self.map_widget.set_marker(lat_ch, lon_ch, text=position_name)
            self.target_markers[position_name] = (None, new_marker)  # Нову мітку зберігаємо в словнику

            # Оновлюємо координати в базі даних (якщо необхідно)
            self.db.execute_query("UPDATE position SET lat_ch = ?, long_ch = ? WHERE name = ?", (lat_ch, lon_ch, position_name))

            print(f"Позиція {position_name} перекатувалась на нові координати: {lat_ch}, {lon_ch}.")
        else:
            print(f"Не вдалося знайти координати для позиції: {position_name}")
